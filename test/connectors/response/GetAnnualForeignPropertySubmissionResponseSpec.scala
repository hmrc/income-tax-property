...
class GetAnnualForeignPropertySubmissionResponseSpec extends UnitTest {
  ...
  private val underTest = getAnnualForeignPropertySubmissionDataReads

  "getAnnualForeignPropertySubmissionDataReads" should {

    "convert JsValue to GetAnnualForeignPropertySubmissionResponse" when {

      "status is OK and valid jsValue" in {
        val annualForeignPropertySubmission = AnnualForeignPropertySubmission(
          None
        )

        val httpResponse: HttpResponse =
          HttpResponse.apply(OK, Json.toJson(annualForeignPropertySubmission).toString, anyHeaders)

        underTest.read(anyMethod, anyUrl, httpResponse) shouldBe GetAnnualForeignPropertySubmissionResponse(
          httpResponse,
          Right(Some(annualForeignPropertySubmission))
        )
      }
      ...
      "status is OK and invalid jsValue" ignore {
        val jsValue: JsValue = Json.parse("""
                                            |{
                                            |   "AnnualForeignPropertySubmission": {"value": []}
                                            |}
                                            |""".stripMargin)
