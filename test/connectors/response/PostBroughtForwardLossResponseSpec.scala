...
import connectors.response.PostBroughtForwardLossResponse.postBroughtForwardLoss
...
class PostBroughtForwardLossResponseSpec extends UnitTest {
  ...
  private val underTest = postBroughtForwardLoss

  val broughtForwardLossId: BroughtForwardLossId = BroughtForwardLossId(lossId = "some-id")

  "postBroughtForwardLoss" should {
    "convert JsValue to BroughtForwardLossId" when {

      "status is OK and valid jsValue" in {
        val httpResponse: HttpResponse = HttpResponse.apply(OK, Json.toJson(broughtForwardLossId), anyHeaders)
        underTest.read(anyMethod, anyUrl, httpResponse) shouldBe PostBroughtForwardLossResponse(
          httpResponse,
          Right(broughtForwardLossId)
        )
      }
