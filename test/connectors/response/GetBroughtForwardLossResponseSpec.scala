...
import connectors.response.GetBroughtForwardLossResponse.getBroughtForwardLoss
...
class GetBroughtForwardLossResponseSpec extends UnitTest {
  ...
  private val underTest = getBroughtForwardLoss
  ...
  "getBroughtForwardLoss" should {
    "convert JsValue to BroughtForwardLoss" when {
      ...
      "status is OK and valid jsValue" in {
        ...
        underTest.read(anyMethod, anyUrl, httpResponse) shouldBe GetBroughtForwardLossResponse(
          httpResponse,
