...
import connectors.response.GetForeignIncomeSubmissionResponse.getForeignIncomeSubmissionDataReads
...
class GetForeignIncomeSubmissionResponseSpec extends UnitTest {
  ...
  private val underTest = getForeignIncomeSubmissionDataReads

  val dividendsIncomeResponse: Option[ForeignIncomeSubmission] = Some(
    emptyForeignIncomeSubmission.copy(
  ...
  "GetForeignIncomeSubmissionResponse" should {
    "convert JsValue to Option[DividendsIncomeSubmission]" when {
      "status is OK and valid jsValue" in {
        val httpResponse: HttpResponse = HttpResponse.apply(OK, Json.toJson(dividendsIncomeResponse), anyHeaders)
        underTest.read(anyMethod, anyUrl, httpResponse) shouldBe GetForeignIncomeSubmissionResponse(
          httpResponse,
          Right(dividendsIncomeResponse)
        )
      }
