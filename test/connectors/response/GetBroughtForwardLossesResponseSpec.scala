...
import connectors.response.GetBroughtForwardLossesResponse.getBroughtForwardLosses
import models.LossType.UKProperty
import models.errors.{ApiError, SingleErrorBody}
import models.responses.{BroughtForwardLossResponseWithId, BroughtForwardLosses}
...
class GetBroughtForwardLossesResponseSpec extends UnitTest {
  ...
  private val underTest = getBroughtForwardLosses

  val broughtForwardLosses: BroughtForwardLosses = BroughtForwardLosses(
    losses = Seq(
  ...
  "getBroughtForwardLosses" should {
    "convert JsValue to BroughtForwardLosses" when {

      "status is OK and valid jsValue" in {
        val httpResponse: HttpResponse = HttpResponse.apply(OK, Json.toJson(broughtForwardLosses), anyHeaders)
        underTest.read(anyMethod, anyUrl, httpResponse) shouldBe GetBroughtForwardLossesResponse(
          httpResponse,
      ...
      "status is NOT_FOUND and any jsValue" in {
        ...
        underTest.read(anyMethod, anyUrl, httpResponse) shouldBe GetBroughtForwardLossesResponse(
          httpResponse,
          Right(BroughtForwardLosses(List()))
        )
      }
