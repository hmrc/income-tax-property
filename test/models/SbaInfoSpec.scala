...
class SbaInfoSpec extends UnitTest {
  ...
  "SbaInfo" should {
    "deserialise the json" in {
      ...
      validRequestBody.as[SbaInfo] shouldBe sbaInfo
  ...
  "SbaInfoExtension" should {
    ...
    "convert from sbaInfo to sbas" in {
      val sbas = sbaInfo.sbas.map(sbaInRequest => StructuredBuildingAllowance(
        sbaInRequest.sbaClaim,
        Some(StructuredBuildingAllowanceDate(sbaInRequest.sbaQualifyingDate, sbaInRequest.sbaQualifyingAmount)),
    ...
    "convert to from SbaInfo to SbaInfoToSave" in {
      sbaInfo.toSbaToSave shouldBe SbaInfoToSave(sbaInfo.claimStructureBuildingAllowance, sbaInfo.sbaClaims)
    }
  }
