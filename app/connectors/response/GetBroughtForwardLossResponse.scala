...
object GetBroughtForwardLossResponse {
  implicit val getBroughtForwardLoss: HttpReads[GetBroughtForwardLossResponse] = new HttpReads[GetBroughtForwardLossResponse] with Parser {
    ...
    override def read(method: String, url: String, response: HttpResponse): GetBroughtForwardLossResponse = response.status match {
      case OK => GetBroughtForwardLossResponse(response, extractResult(response))
      ...
      case BAD_REQUEST | UNPROCESSABLE_ENTITY | INTERNAL_SERVER_ERROR | SERVICE_UNAVAILABLE =>
        GetBroughtForwardLossResponse(response, handleError(response, response.status))
