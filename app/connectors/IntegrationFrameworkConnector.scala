...
class IntegrationFrameworkConnector @Inject() (httpClient: HttpClient, appConf: AppConfig)(implicit
  ...
  @deprecated
  def createOrUpdateAnnualSubmission(taxYear: Int, nino: String, incomeSourceId: String, body: JsValue)(implicit
    hc: HeaderCarrier
  ): Future[Either[ApiError, Unit]] = {
    val (url, apiVersion) = if (after2324Api(taxYear)) {
      (
      ...
      (
        url"""${appConfig.ifBaseUrl}/income-tax/business/property/annual?taxableEntityId=$nino&taxYear=${toTaxYearParamBefore2324(
            taxYear
          )}&incomeSourceId=$incomeSourceId""",
        "1597"
      )
    ...
    httpClient
      .PUTString[PutAnnualSubmissionResponse](url, StaticBinding.generateFromJsValue(body, escapeNonASCII = false))(
