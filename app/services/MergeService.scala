...
import models.request.foreign.{ForeignIncome, ForeignPropertySelectCountry, ForeignPropertyTax}
...
class MergeService @Inject() (implicit
  ...
  def mergeAll(
    ...
    val foreignPropertyIncomeMaybe = mergeForeignPropertyIncome(
      resultFromPeriodicDownstreamMaybe,
      foreignResultFromRepository.get(JourneyName.ForeignIncomeJourney.entryName)
    )
