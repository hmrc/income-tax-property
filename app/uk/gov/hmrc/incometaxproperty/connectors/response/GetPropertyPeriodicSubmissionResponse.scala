...
case class GetPropertyPeriodicSubmissionResponse(httpResponse: HttpResponse, result: Either[ApiError, Option[PropertyPeriodicSubmission]])

object GetPropertyPeriodicSubmissionResponse {

  implicit val getPropertyPeriodicSubmissionDataReads: HttpReads[GetPropertyPeriodicSubmissionResponse] =
    new HttpReads[GetPropertyPeriodicSubmissionResponse] with Parser {
      ...
      override def read(method: String, url: String, response: HttpResponse): GetPropertyPeriodicSubmissionResponse = response.status match {
        ...
        case NOT_FOUND => GetPropertyPeriodicSubmissionResponse(response, Right(None))
      ...
      private def extractResult(response: HttpResponse): Either[ApiError, Option[PropertyPeriodicSubmission]] = {
        val json = response.json
        json.validate[PropertyPeriodicSubmission]
          .fold[Either[ApiError, Option[PropertyPeriodicSubmission]]](_ => badSuccessJsonResponse, parsedModel => Right(Some(parsedModel)))
      }
    }
}
