...
object PropertyPeriodicSubmissionRequest {
  ...
  def fromUkOtherPropertyIncome(
    ...
    val result: PropertyPeriodicSubmissionRequest = updateUkPropertyRentalsIncome(ukOtherPropertyIncome, requestWithEmptyRentalsIncome)
  ...
  private def updateUkPropertyRentalsIncome(
    ...
    val ukOtherPropertyLens: Optional[PropertyPeriodicSubmissionRequest, UkOtherProperty] = Optional[PropertyPeriodicSubmissionRequest, UkOtherProperty] {
      ppsr =>
        ppsr match {
      ...
      ukop => ppsr => ppsr.copy(ukOtherProperty = Some(ukop))
    }

    val ukOtherPropertyIncomeLens: Optional[UkOtherProperty, UkOtherPropertyIncome] = Optional[UkOtherProperty, UkOtherPropertyIncome] {
      ukop =>
        ukop match {
      ...
      ukopi => ukop => ukop.copy(income = Some(ukopi))
    }
